...
export default function CompanyDocumentsClientPage({ initialDocuments }: CompanyDocumentsClientPageProps) {
    const [documents, setDocuments] = useState<ManagedDocument[]>(initialDocuments);
    const [isLoading, setIsLoading] = useState(initialDocuments.length === 0); // Only load if no initial data
    const { toast } = useToast();

    useEffect(() => {
        // Only fetch if initial data wasn't provided from the server
        if (initialDocuments.length === 0) {
            async function fetchData() {
                setIsLoading(true);
                try {
                    const docs = await getGeneralDocuments();
                    setDocuments(docs);
                } catch (error) {
