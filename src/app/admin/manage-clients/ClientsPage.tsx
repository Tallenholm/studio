...
export default function ClientsPage({ initialClients, allJobs }: ClientsPageProps) {
  ...
  async function onSubmit(values: z.infer<typeof clientSchema>) {
    if (editingClient) {
      ...
      // When adding a new client, stats will be 0
      const newClient = { id: newClientId, ...values, jobCount: 0, totalValue: 0 };
  ...
  async function removeClient(clientId: string) {
    const clientHasJobs = allJobs.some(job => job.clientId === clientId);
    if (clientHasJobs) {
      toast({
        variant: 'destructive',
