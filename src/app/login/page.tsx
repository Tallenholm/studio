  ...
  return (
    <div className="flex items-center justify-center min-h-screen p-4 relative overflow-hidden">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, hsl(224 71% 4%) 0%, hsl(215 28% 12%) 50%, hsl(195 40% 8%) 100%)',
        }}
      />
      <div
        className="absolute inset-0 -z-10 opacity-30"
        style={{
          background: 'radial-gradient(ellipse at 20% 50%, hsl(195 100% 50% / 0.15) 0%, transparent 50%), radial-gradient(ellipse at 80% 20%, hsl(38 92% 50% / 0.1) 0%, transparent 50%)',
        }}
      />

      <Card className="w-full max-w-sm glass-card shadow-2xl border-white/10 animate-fade-in-up">
        <CardHeader className="text-center items-center">
          <div className="p-3 bg-primary/10 border border-primary/20 rounded-full mb-2 shadow-lg shadow-primary/10">
        ...
        <CardContent>
          {!isFirebaseConfigured ? (
            <div className="flex flex-col items-center justify-center gap-2 text-sm text-destructive p-4 bg-destructive/10 border border-destructive/30 rounded-lg">
            ...
            <Form {...loginForm}>
              <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4 pt-4">
                <fieldset disabled={isFormDisabled} className="space-y-4 disabled:opacity-60">
                  <FormField control={loginForm.control} name="email" render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl><Input placeholder="name@company.com" {...field} aria-label="Email address" /></FormControl>
                      <FormMessage />
                    </FormItem>
                  )} />
                  <div>
                    <FormField control={loginForm.control} name="password" render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl><Input type="password" placeholder="••••••••" {...field} aria-label="Password" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                ...
                {loginError && (
                  <div className="flex items-center gap-2 text-sm text-destructive p-3 bg-destructive/10 border border-destructive/30 rounded-lg animate-fade-in-up">
          ...
          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-white/10" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-card/90 px-2 text-muted-foreground">Or continue with</span></div></div>
          <Button variant="outline" className="w-full border-white/10 hover:bg-white/5" onClick={handleGoogleSignIn} disabled={isFormDisabled}>
            {isUserLoading ? (
              <Loader2 className="mr-2 h-5 w-5 animate-spin" />
            ) : (
