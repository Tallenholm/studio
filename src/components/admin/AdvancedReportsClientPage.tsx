...
interface AdvancedReportsClientPageProps {
  initialData: AdvancedReportData;
}

export default function AdvancedReportsClientPage({ initialData }: AdvancedReportsClientPageProps) {
  const [reportData, setReportData] = useState<AdvancedReportData | null>(initialData);
  ...
  useEffect(() => {
    // We already have initial data, so we don't need to fetch on mount.
    // This effect will only run when the filters are changed by the user.
    const isDefaultFilter = dateRangeFilter === 'all_time' && vehicleTypeFilter === 'all';

    if (!isDefaultFilter) {
      fetchReportData();
    } else {
      // If user resets to default filters, use the initial server-provided data.
      setReportData(initialData);
    }
  }, [dateRangeFilter, vehicleTypeFilter, fetchReportData, initialData]);
        ...
        <CardContent className="space-y-8">
          <Card className="bg-muted/30">
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={dateRangeFilter} onValueChange={(val: 'all_time' | 'last_30_days' | 'last_quarter') => setDateRangeFilter(val)}>
              ...
              <div>
                ...
                <Select value={vehicleTypeFilter} onValueChange={(value: VehicleType | 'all') => setVehicleTypeFilter(value)}>
          ...
          {isLoading ? (
            <div className="flex flex-col justify-center items-center min-h-[400px]">
              ...
              <p className="text-lg text-muted-foreground">Updating Reports...</p>
            </div>
          ) : !reportData?.hasData ? (
            <div className="text-center py-10 border-2 border-dashed rounded-lg">
              ...
              <p className="text-lg text-muted-foreground">
                No data available to generate reports for the selected filters.
              </p>
            </div>
          ) : (
