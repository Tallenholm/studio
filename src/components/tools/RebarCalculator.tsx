...
export default function RebarCalculator() {
  ...
  const OVERLAP_FACTOR = 1.1; // 10% for overlap
  ...
  const calculate = () => {
    ...
    if (isNaN(L) || isNaN(W) || isNaN(S) || L <= 0 || W <= 0 || S <= 0) {
      setResult(null);
      return;
    }
    ...
    const totalFeet = (lenRows + lenCols) * OVERLAP_FACTOR;
    ...
    <div className="space-y-4">
        ...
        {result !== null && (
          <div className="text-center pt-2">
            ...
            <p className="text-xs text-muted-foreground">(includes 10% for overlap)</p>
          </div>
        )}
    </div>
  );
