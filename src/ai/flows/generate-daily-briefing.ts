...
const generateBriefingFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const prompt = `You are an AI assistant for an operations manager at an excavating company. Your task is to create a daily briefing based on the provided data. It is currently ${new Date(input.date).toDateString()}.

    Analyze the provided data for jobs, failed inspection reports, pending requests (time off, expenses), pending tasks, and today's events.
    
    Categorize the information into three lists:
    1.  **attentionItems**: High-priority items. This should exclusively be for FAILED inspection reports. For each, create a summary like "Failed pre-trip report for [Asset Name] by [Employee Name]." The link should be '/admin/reports/[reportId]'.
    ...
    Here is the data:
    - Active Jobs: ${JSON.stringify(input.jobs)}
    - Failed Reports: ${JSON.stringify(input.reports)}
    - Pending Time Off: ${JSON.stringify(input.timeOffRequests)}
    - Pending Expenses: ${JSON.stringify(input.expenseReports)}
    - Pending Tasks: ${JSON.stringify(input.tasks)}
    - Today's Events: ${JSON.stringify(input.events)}
    `;
