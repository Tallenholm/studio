...
const CreateJobFromPromptOutputSchema = z.object({
  ...
  jobValue: z.number().optional().describe('The total monetary value of the job. Should be a number.'),
...
Today's date is {{{currentDate}}}. Use this as a reference for relative dates like "tomorrow" or "next week".

Analyze the following prompt and extract the details for the job. Infer job names from the description if not explicitly provided. Ensure dates are in 'YYYY-MM-DD' format.
